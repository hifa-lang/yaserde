...
//!    };
//!
//!     println!("{}", hifa_yaserde::ser::to_string_with_config(&dev, &yaserde_cfg).ok().unwrap());
//! ```
//!
...
macro_rules! __derive_debug {
  ($($arg:tt)+) => { ::hifa_yaserde::__log::debug!(target: "yaserde_derive", $($arg)+) };
}
...
macro_rules! __derive_trace {
  ($($arg:tt)+) => { ::hifa_yaserde::__log::trace!(target: "yaserde_derive", $($arg)+) };
}
...
macro_rules! deserialize_and_validate {
  ($content: expr, $model: expr, $struct: tt) => {
    ...
    let loaded: Result<$struct, String> = hifa_yaserde::de::from_str($content);
